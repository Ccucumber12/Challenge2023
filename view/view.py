...
<<<<<<< HEAD
            ...
            self.character_image[player][const.CharacterDirection.DOWN] = crop(
                picture, const.PLAYER_RADIUS * 2, const.PLAYER_RADIUS * 2, True
            )
            picture = pg.image.load(
                os.path.join(
                    const.PICTURES_PATH[player],
            ...
            self.character_image[player][const.CharacterDirection.LEFT] = crop(
                picture,
                const.WINDOW_SIZE[0],
                self.character_image[player][
            ...
            self.character_image[player][const.CharacterDirection.UP] = crop(
                picture,
                const.WINDOW_SIZE[0],
                self.character_image[player][
                    const.CharacterDirection.DOWN
            ...
            self.character_image[player][const.CharacterDirection.RIGHT] = crop(
                picture,
                const.WINDOW_SIZE[0],
                self.character_image[player][
                    const.CharacterDirection.DOWN
...
>>>>>>> 209d0eb (refactor: move scoreboard into view.objects)
            ...
            for direction in const.CharacterDirection:
                self.transparent_player_image[player][direction] = self.character_image[
                    player
                ][direction].convert_alpha()
...
<<<<<<< HEAD
                ...
                imgdic[player][const.CharacterDirection.LEFT] = crop(
                    picture,
                    const.WINDOW_SIZE[0],
                ...
                imgdic[player][const.CharacterDirection.UP] = crop(
                    picture,
                    const.WINDOW_SIZE[0],
                    imgdic[player][const.CharacterDirection.DOWN].get_height(),
                )
                ...
                imgdic[player][const.CharacterDirection.RIGHT] = crop(
                    picture,
                    const.WINDOW_SIZE[0],
                    imgdic[player][const.CharacterDirection.DOWN].get_height(),
                )
...
>>>>>>> 209d0eb (refactor: move scoreboard into view.objects)
            ...
            # sortinghat
            load_player_skin(
                player, self.wearing_sortinghat_image, const.PlayerSkins.SORTINGHAT
            )
...
<<<<<<< HEAD
                picture = pg.image.load(
                    os.path.join(
                        ...
                        const.PICTURES_PATH[direction],
                    )
                ).convert_alpha()
                self.shining_player_image[player][direction] = crop(
                    picture, *const.SHINING_PLAYER_SIZE[direction], True
                )
...
<<<<<<< HEAD
                    ...
                    self.character_image[ghost].append(
                        crop(
                            picture,
        ...
        picture = pg.image.load(
            os.path.join(const.PICTURES_PATH[const.GhostSkins.KILLING], "right.png")
        ).convert_alpha()
        self.ghost_killing_image[const.CharacterDirection.RIGHT] = crop(
            picture, const.GHOST_RADIUS * 2, const.GHOST_RADIUS * 2, True
        )
        picture = pg.image.load(
            os.path.join(const.PICTURES_PATH[const.GhostSkins.KILLING], "left.png")
        ).convert_alpha()
...
>>>>>>> 209d0eb (refactor: move scoreboard into view.objects)
    ...
    def add_patronus_shockwave_animation(self, event):
        ...
        self.patronus_shockwave_animations.append(
            (event.position, model.timer + duration, duration)
        )
...
>>>>>>> fb322f5 (refactor: move items into view.objects)
        for player in model.players:
            ...
            detail = (
                ...
                player.dead,
                player.effect_timer,
                player.respawn_time - model.timer,
            )
...
>>>>>>> 1510e82 (refactor: move players into view.objects)
        for ghost in model.ghosts:
            ...
            objects.append(
                Object(
                    coord[1],
                    const.ObjectType.GHOST,
                    ghost.position,
                    const.GhostIds.DEMENTOR,
                    (ghost.ghost_id,),
                )
            )
...
<<<<<<< HEAD
                ...
                if dead:
                    if (
                        ...
                        or direction == const.CharacterDirection.UP
                    ):
                        render_picture(
                            self.dead_player_image, obj.image_index, direction, obj
                        )
                    if direction == const.CharacterDirection.LEFT:
                        width = self.dead_player_image[obj.image_index][
                            direction
                        ].get_width()
                        height = self.dead_player_image[obj.image_index][
                            direction
                        ].get_height()
                        ul = [
                            x - y for x, y in zip(obj.position, [width / 2 - 7, height])
                        ]
                        self.screen.blit(
                            self.dead_player_image[obj.image_index][direction], ul
                        )
                    ...
                    rect = pg.Rect((obj.position + pg.Vector2(0, -30)), (0, 0)).inflate(
                        radius, radius
                    )
                    pg.draw.arc(
                        self.screen,
                        const.PLAYER_COLOR[obj.image_index],
                        rect,
                        pi / 2 - angle,
                        pi / 2,
                        width=6,
                    ...
                    effect_timer < const.ITEM_LOSE_EFFECT_HINT_TIME
                    and quarter_sec % 2 == 0
                ):
                    self.screen.blit(
                    ...
                    self.screen.blit(
                        self.petrified_player_image[obj.image_index][direction], ul
                    )
                elif effect == const.EffectType.CLOAK:
                    self.screen.blit(
                        self.transparent_player_image[obj.image_index][direction], ul
                    )
                elif effect == const.EffectType.SORTINGHAT:
                    width = self.wearing_sortinghat_image[obj.image_index][
                        direction
                    ].get_width()
                    height = self.wearing_sortinghat_image[obj.image_index][
                        direction
                    ].get_height()
                    ul = [x - y for x, y in zip(obj.position, [width / 2, height])]
                    self.screen.blit(
...
<<<<<<< HEAD
                    ...
                    height = (
                        self.character_image[obj.image_index][direction].get_height()
                        + self.shining_player_image[obj.image_index][
                            direction
                        ].get_height()
                    ) / 2
                    ul = [x - y for x, y in zip(obj.position, [width / 2, height])]
...
>>>>>>> 1510e82 (refactor: move players into view.objects)
                ...
                ul = [
                    x - y
                    for x, y in zip(
                        obj.position, [const.GHOST_RADIUS, const.GHOST_RADIUS * 2]
                    )
                ]
                ...
                for kill_animation in self.ghost_kill_animations:
                    if kill_animation[0] == obj.detail[0]:
                        if obj.position.x < kill_animation[1].x:
                            ...
                            self.screen.blit(
                                self.ghost_killing_image[const.CharacterDirection.LEFT],
                                ul,
                            )
                        ghost_shown = True
                        break
                if not ghost_shown:
                    self.screen.blit(
                        self.character_image[const.GhostIds.DEMENTOR][
                            model.timer
                ...
                if (
                    quarter_sec % 2 == 0
                    or model.timer + const.ITEM_LOSE_EFFECT_HINT_TIME <= effect_timer
                ):
                    ul = [
                        x - y
                        for x, y in zip(
...
<<<<<<< HEAD
                ...
                if ripple.show:
                    ...
                    self.screen.blit(
                        ripple_surface,
                        pg.Rect(ripple.position, (0, 0)).inflate(ripple.size),
                    )
                ...
                ul = [
                    x - y
                    for x, y in zip(
                        obj.position, [const.ITEM_RADIUS, const.ITEM_RADIUS * 2]
                    )
                ]
        ...
        for animation in animations:
            ...
            for i in range(5):
                radius = const.PATRONUS_SHOCKWAVE_RADIUS * (
                    1 - min(1, (disappear_time - model.timer + i) / duration)
                )
    ...
    def render_endgame(self):
        ...
        for i in range(len(self.places)):
            ...
            for player in self.places[i]:
                if i == 3:
                    surface = self.character_image[player.player_id][
